    ...
    <tbody>
        ...
        <?php foreach($tasks as $task): ?>
            <tr>
                ...
                <!--No usa htmlspecialchars porque este dato no lo genera el usuario -->
...
osea pasa de falso a verdadero y de verdadero a falso-->      
                ...
                <td>
<!--se crea un formulario para enviar datos por medio de POST-->
                <form action="../controllers/task_delete.php" method="POST">
<!--se envia de manera oculta el valor de task en su atributo id-->
                    <input type="hidden" name="id" value="<?= $task['id'] ?>">
<!--se envia de manera oculta el valor de user_id en su atributo id-->                    
                    <input type="hidden" name="user_id" value="<?= $user['id'] ?>">
                <!--Su entregan los datos anteriores al controlador delete.php
            on onlick como event handler de JavaScript cuando se oprime el boton eliminar
            muestra el texto, return retorna false o true si estru envia los datos al controller
                sino no envia nada-->
                    <button type="submit" onclick="return confirm('¿Confirmas que deseas eliminar?')" >Eliminar</button>      
                </form>
